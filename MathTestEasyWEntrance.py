...
class QuestionEngine:
    ...
    def _tier2():
        ...
        if kind == 'two_step':
            ...
            sub = random.choice(['coins', 'items', 'age'])
            if sub == 'coins':
                quarters = random.randint(1, 6)
                dimes    = random.randint(1, 8)
                ans      = quarters * 25 + dimes * 10
                q        = ("You have {} quarter(s) and {} dime(s).\n"
                            "How many cents do you have?").format(quarters, dimes)
            elif sub == 'items':
    ...
    def _toggle_button(self):
        if self._consent_var.get():
            self._start_btn.config(
                state=tk.NORMAL, bg=IN_ACCENT, fg=IN_BTN_FG)
        else:
            self._start_btn.config(
                state=tk.DISABLED, bg="#aaaaaa", fg="#dddddd")
...
class MathTestUI:
    ...
    def _build_ui(self):
        ...
        tier_text, tier_bg, tier_fg = self.TIER_INFO[1]
        self.tier_label = tk.Label(
            top, text=tier_text, bg=tier_bg, fg=tier_fg,
    ...
    def generate_question(self):
        ...
        try:
            ...
            q_text, correct, choices = QuestionEngine.generate(self.tier)
    ...
    def write_header_to_csv(self):
        ...
        with open(self.path_to_file, 'a', newline='') as f:
            w = csv.writer(f)
            # Participant metadata rows at the top
...
def launch_test(participant_data):
    test_root = tk.Tk()
    sw = test_root.winfo_screenwidth()
    sh = test_root.winfo_screenheight()
    ww = max(900,  min(1920, int(sw * 0.90)))
    wh = max(650,  min(1080, int(sh * 0.90)))
    test_root.geometry("{}x{}+{}+{}".format(ww, wh, (sw - ww) // 2, (sh - wh) // 2))
    test_root.minsize(900, 650)
    MathTestUI(test_root,
